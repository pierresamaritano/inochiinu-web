...
export default function PensionPage() {
  ...
  // MODIFICATION : Minuteur intelligent pour le carrousel principal
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      setTimeToNext(6000); // Retour à la vitesse normale (6s) après un passage automatique
    }, timeToNext);
    return () => clearTimeout(timer);
  }, [currentSlide, timeToNext, slides.length]);

  const handleUserInteraction = () => setTimeToNext(12000); // Pause de 12s au clic/tapotement
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("pension_requests").insert([{
        user_id: user.id, dog_id: formData.dog_id, client_name: user.user_metadata?.full_name || "Client",
        client_email: user.email, client_phone: formData.clientPhone, dog_name: finalDogName,
        dog_breed: finalDogBreed, start_date: formData.startDate, end_date: formData.endDate,
        special_needs: formData.specialNeeds, status: "en_attente",
      }]);
