...
export default function SelleriePage() {
  ...
  const [formData, setFormData] = useState({
    itemType: "Laisse Modulaire Multi-positions",
    colorFinish: "Fauve & Quincaillerie Noir Mat",
    dogSize: "Grand Chien (Akita / Berger)",
    customDetails: "",
    clientPhone: "",
  });
  ...
  const slides = [
    {
      title: "Matériaux Techniques Haute Résistance",
      subtitle: "Sangles biothane / polypropylène imputrescibles et bouclerie en acier forgé testée aux fortes tractions.",
      tag: "Robustesse Extrême",
      gradient: "from-amber-950/90 via-stone-900/60 to-black/80",
    },
    {
      title: "Confection Artisanale à la Main",
      subtitle: "Chaque pièce est cousue et assemblée dans notre atelier avec un soin du détail et des finitions durables.",
      tag: "Fait Main en France",
      gradient: "from-stone-900/90 via-stone-900/60 to-black/80",
    },
    {
      title: "Suivi d'Atelier en Temps Réel",
      subtitle: "Suivez la confection de votre commande : découpe, couture, assemblage et envoi sur votre tableau de bord.",
      ...
      const { error } = await supabase.from("sellerie_orders").insert([
        {
          ...
          color_finish: formData.colorFinish,
    ...
    <div className="relative min-h-screen bg-[#FDFCF8] text-stone-800 antialiased selection:bg-orange-200 selection:text-stone-900">
      ...
      <section className="relative z-10 flex w-full flex-col items-center pt-36 pb-6 text-center px-4">
        <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-orange-500/30 bg-orange-50/70 backdrop-blur-md px-4 py-1 text-xs font-bold text-orange-700 shadow-sm">
          <span>Artisanat Canin & Équipements</span>
        </div>
        ...
        <p className="mx-auto mt-3 max-w-2xl text-sm font-medium text-stone-600 sm:text-base">
          Équipements ultra-résistants confectionnés à la main, adaptés aux chiens puissants et aux promenades exigeantes.
        </p>
      </section>
          ...
          {slides.map((slide, index) => (
            ...
            >
              ...
              <div className="relative z-10 max-w-xl text-white">
                <span className="text-[10px] font-black uppercase tracking-wider text-amber-400 bg-white/10 backdrop-blur-md px-3 py-0.5 rounded-full border border-white/10 inline-block mb-2">
                  {slide.tag}
                </span>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6 p-6 sm:p-8 rounded-[2rem] bg-white/80 backdrop-blur-xl border border-stone-200/80 shadow-sm">
          <div>
            <h2 className="text-xl font-black tracking-tight text-stone-900">
              Commander un équipement sur-mesure
            </h2>
            <p className="mt-1 text-xs sm:text-sm text-stone-500 font-medium">
              Configurez votre laisse, longe ou harnais et suivez la fabrication en direct depuis votre espace.
            </p>
          </div>
          <button
        ...
        <div className="mx-auto max-w-5xl px-6 space-y-12">
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl font-black tracking-tight text-stone-900 mt-1">
              Conçus pour l'aventure et la sécurité
            </h2>
          </div>

          <div className="grid gap-6 md:grid-cols-3">
            <div className="rounded-[2rem] border border-stone-200/80 bg-white/60 backdrop-blur-xl p-8 shadow-sm">
              <span className="text-[10px] font-black uppercase text-orange-600 bg-orange-50 px-2.5 py-1 rounded-full">Polyvalent</span>
              <h3 className="mt-4 text-lg font-bold text-stone-900">Laisses Modulaires</h3>
              <p className="mt-2 text-xs text-stone-500">3 points de réglage : port en bandoulière, tour de taille ou laisse courte de ville.</p>
            </div>
            <div className="rounded-[2rem] border border-stone-200/80 bg-white/60 backdrop-blur-xl p-8 shadow-sm">
              <span className="text-[10px] font-black uppercase text-orange-600 bg-orange-50 px-2.5 py-1 rounded-full">Travail & Liberté</span>
              <h3 className="mt-4 text-lg font-bold text-stone-900">Longes Éducatives</h3>
              <p className="mt-2 text-xs text-stone-500">5m, 10m ou 15m légères et résistantes, sans nœuds, idéales pour le travail du rappel.</p>
            </div>
            <div className="rounded-[2rem] border border-stone-200/80 bg-white/60 backdrop-blur-xl p-8 shadow-sm">
              <span className="text-[10px] font-black uppercase text-orange-600 bg-orange-50 px-2.5 py-1 rounded-full">Sécurité</span>
              <h3 className="mt-4 text-lg font-bold text-stone-900">Colliers & Harnais</h3>
              <p className="mt-2 text-xs text-stone-500">Ajustements anatomiques pour éviter toute gêne articulaire et maximiser le confort.</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md rounded-[2.5rem] border border-white/80 bg-[#FDFCF8]/95 p-8 sm:p-10 shadow-2xl backdrop-blur-2xl">
            ...
            <div className="text-center">
              ...
              <p className="mt-2 text-sm text-stone-500 font-medium">Connectez-vous pour passer commande et suivre l'avancement en atelier.</p>
            </div>
          ...
          <div className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-[2.5rem] border border-white/80 bg-[#FDFCF8] p-6 sm:p-10 shadow-2xl">
            ...
            {submitted ? (
              <div className="text-center py-8">
                ...
                <h3 className="text-xl font-black text-stone-900">Commande transmise à l'atelier !</h3>
                <p className="text-xs text-stone-500 mt-2">Votre demande d'équipement personnalisé est enregistrée.</p>
                <a href="/espace-membre" className="mt-6 inline-block px-6 py-2.5 bg-stone-900 text-white font-bold text-xs rounded-full">Suivre sur mon espace</a>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="flex items-center justify-between pb-4 border-b border-stone-100">
                  <div>
                    ...
                    <h3 className="text-lg font-black text-stone-900">{step === 1 ? "Équipement & Finition" : "Gabarit & Validation"}</h3>
                  </div>
                </div>
                ...
                {step === 1 && (
                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-bold uppercase text-stone-600 mb-1">Article souhaité</label>
                      <select value={formData.itemType} onChange={(e) => setFormData({ ...formData, itemType: e.target.value })} className="w-full px-4 py-3 rounded-2xl bg-stone-50 border border-stone-200 text-xs font-medium">
                        <option value="Laisse Modulaire Multi-positions (2m50)">Laisse Modulaire Multi-positions (2m50)</option>
                        <option value="Longe de Rappel (10 mètres)">Longe de Rappel (10 mètres)</option>
                        <option value="Collier Confort Renforcé">Collier Confort Renforcé</option>
                        <option value="Set Complet Promenade">Set Complet Promenade (Laisse + Collier)</option>
                      </select>
                    </div>

                    <div>
                      <label className="block text-xs font-bold uppercase text-stone-600 mb-1">Coloris / Finitions</label>
                      <select value={formData.colorFinish} onChange={(e) => setFormData({ ...formData, colorFinish: e.target.value })} className="w-full px-4 py-3 rounded-2xl bg-stone-50 border border-stone-200 text-xs font-medium">
                        <option value="Fauve / Bouclerie Noir Mat">Fauve / Bouclerie Noir Mat</option>
                        <option value="Kaki Tactique / Bouclerie Laiton">Kaki Tactique / Bouclerie Laiton</option>
                        <option value="Noir Intense / Bouclerie Noir Mat">Noir Intense / Bouclerie Noir Mat</option>
                      </select>
                    </div>

                    <div className="pt-4 flex justify-end">
                      <button type="button" onClick={() => setStep(2)} className="px-6 py-3 bg-stone-900 text-white font-bold text-xs rounded-full cursor-pointer">Suivant ➔</button>
                    </div>
                  </div>
                )}
                ...
                {step === 2 && (
                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-bold uppercase text-stone-600 mb-1">Gabarit du chien</label>
                      <select value={formData.dogSize} onChange={(e) => setFormData({ ...formData, dogSize: e.target.value })} className="w-full px-4 py-3 rounded-2xl bg-stone-50 border border-stone-200 text-xs font-medium">
                        <option value="Grand Chien (Akita / Berger - 30kg+)">Grand Chien (Akita / Berger - 30kg+)</option>
                        <option value="Chien Moyen (Shiba / Border - 10 à 20kg)">Chien Moyen (Shiba / Border - 10 à 20kg)</option>
                        <option value="Petit Chien / Chiot (-10kg)">Petit Chien / Chiot (-10kg)</option>
                      </select>
                    </div>
                    ...
                    <div>
                      <label className="block text-xs font-bold uppercase text-stone-600 mb-1">Précisions ou gravure souhaitée</label>
                      <textarea rows={2} placeholder="Tour de cou exact, personnalisation..." value={formData.customDetails} onChange={(e) => setFormData({ ...formData, customDetails: e.target.value })} className="w-full px-4 py-2.5 rounded-2xl bg-stone-50 border border-stone-200 text-xs font-medium" />
                    </div>

                    <div className="pt-4 flex justify-between items-center">
                      <button type="button" onClick={() => setStep(1)} className="text-xs font-bold text-stone-500">← Retour</button>
                      <button type="submit" disabled={submitting} className="px-6 py-3 bg-gradient-to-tr from-orange-600 to-orange-500 text-white font-black text-xs uppercase rounded-full cursor-pointer">{submitting ? "Envoi..." : "Valider ma commande"}</button>
                    </div>
                  </div>
                )}
              </form>
